<template>
  <div class="layout">
    ...
    <main class="main-content">
      <div class="header">
        <h1>👨‍⚕️ Médicos</h1>
        <button v-if="authStore.esAdmin" class="btn-primary" @click="abrirModal()">+ Nuevo Médico</button>
      </div>

      <input v-model="busqueda" class="buscador" placeholder="🔍 Buscar médico..." />
        ...
        <table>
          ...
          <tbody>
            <tr v-for="m in medicosFiltrados" :key="m._id">
              ...
              <td><span class="badge">{{ m.especialidad }}</span></td>
              <td>{{ m.email }}</td>
              <td>{{ m.telefono }}</td>
              <td>
                <button v-if="authStore.esAdmin" class="btn-edit" @click="abrirModal(m)">✏️</button>
                <button v-if="authStore.esAdmin" class="btn-delete" @click="eliminar(m._id)">🗑️</button>
              </td>
            </tr>
...
.badge { background: #dbeafe; color: #1d4ed8; padding: 0.2rem 0.6rem; border-radius: 20px; font-size: 0.85rem; }
...
.modal { background: white; padding: 2rem; border-radius: 12px; width: 100%; max-width: 600px; }
.modal h2 { margin-bottom: 1.5rem; color: #1e3a5f; }
.form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; }
.form-group { display: flex; flex-direction: column; }
label { font-size: 0.9rem; color: #374151; margin-bottom: 0.3rem; }
input { padding: 0.6rem; border: 1px solid #d1d5db; border-radius: 8px; font-size: 0.95rem; }
